...
Revisions by N3KFL:
December 31, 2015 - Modified to use a Chinese AD9850 module (See http://www.ebay.com/itm/231454411739?_trksid=p2060353.m2749.l2649&ssPageName=STRK%3AMEBIDX%3AIT )  .
January 1, 2016 - This version uses the PJRC encoder library (See http://www.pjrc.com/teensy/td_libs_Encoder.html ).
...
Encoder encoder(2,3);              // Encoder "B" signal to digital pin 2, Encoder "A" signal to digital pin 3 to tune w/ correct sense.
// encoder speed setting. 1 = maximum speed, 2 = half speed etc. This should only be set to a power of 2.
byte _encoderSpeed = 1;            // My encoder = https://www.adafruit.com/products/377
...
// 125,000,000 is 7735940 hex. We make use of the convenient fact that + or - 100 ppm (12.5 KHz) only changes the 2 least significant bytes.
...
unsigned int _clockLSB;  // calibration value. 0x5940 for clock = exactly 125 MHz
...
// setup ports with |= and &= to avoid disturbing serial / USB ports 0 and 1
  ...
  // if it is outside + or - 9999 Hz then set it from 0 Hz, i.e., clock assumed to be exactly  125 MHz
  ...
  ReadEncoder();  // This initializes the static variable prevCount in the ReadEncoder() routine.
...
// read the count into another variable.
  long count = encoder.read();
  
  int diff = (count - prevCount)/4;  // PJRC Encoder library produces 4 counts per step.
...
void PrintClock()
{
  // if _clockLSB == 0x5940 then the offset is zero
  PrintOffset(_clockLSB - 0x5940);
}
...
{
  long clock = CLOCK_BASE | _clockLSB;  // get the clock frequency to use. if _clockLSB is 0x5940 then this is exactly 125 MHz
...
// The following is correct for the AD9850
  WriteByteToDDS(0x00);
